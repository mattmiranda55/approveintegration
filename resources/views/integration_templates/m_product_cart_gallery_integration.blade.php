<script>
    ...
    // Global setup
    var approve_product_wrapper_ele = @json($product->wrapper ?? null);
    var cart_url_pathname = @json($cart->url ?? null);
    var approve_gallery_wrapper_ele = @json($gallery->wrapper ?? null);

    // Driver function to run integration
    function start_approve_integration() {
        // Checks if we are on a product page
        if (approve_product_wrapper_ele
    ...
    function approve_product_page_integration() {
        ...
        var approve_product = {
            model_ele_name: @json($product->model ?? null),
            price_ele_name: @json($product->price ?? null),
            qty_ele_name: @json($product->qty ?? null),
            sku_ele_name: @json($product->sku ?? null),
            insert_after_ele_name: @json($product->ins_aft ?? null),
            insert_after_price_text: @json($product->ins_aft_price ?? null),
            inc_quantity_ele: @json($product->inc_qty ?? null),
            dec_quantity_ele: @json($product->dec_qty ?? null),
    ...
    function approve_cart_page_integration() {
        ...
        var approve_cart_product = {
            wrapper_ele: @json($cart->wrapper ?? null),
            item_ele: @json($cart->item ?? null),
            item_model_ele: @json($cart->item_model ?? null),
            item_price_ele: @json($cart->item_price ?? null),
            item_qty_ele: @json($cart->item_qty ?? null),
            sku: @json($cart->sku ?? null),
            insert_btn_after_ele: @json($cart->ins_btn_aft ?? null),
            global_watcher: @json($cart->wrapper ?? null),
            model_option_ele: @json($cart->model_option ?? null),
            tax_price_ele: @json($cart->tax ?? null),
            shipping_price_ele: @json($cart->shipping ?? null),
            wrapper_align: '',
        ...
        if (approve_gallery_wrapper_ele && document.querySelector(approve_gallery_wrapper_ele)) {
            function init_approve_gallery_page_buttons(gallery_wrapper_ele_name) {
                ...
                var approve_gallery_product = {
                    ...
                    item_ele: gallery_wrapper_ele_name + @json($gallery->item ?? null),
                    item_model_ele_name: @json($gallery->model ?? null),
                    item_price_ele_name: @json($gallery->price ?? null),
                    item_insert_after_ele_name: @json($gallery->ins_aft ?? null),
                    // optional
                    item_qty_ele_name: @json($gallery->qty ?? null),
                    item_inc_quantity_ele_name: @json($gallery->inc_qty ?? null),
                    item_dec_quantity_ele_name: @json($gallery->inc_dec ?? null),
                    item_options_wrapper_ele_name: @json($gallery->optiond ?? null),
